...
only its items move to the strip, and a large title sits inline in the bar and disappears once scrolled. A sheet
that reaches under the strip keeps its own bars and takes the strip's inset, leaving the strip showing only its
status glyphs. Every pose puts the controls on a side edge except the inner display in portrait, the one HIG
exception, which keeps horizontal bars.
